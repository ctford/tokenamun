...
| --- | --- | --- |
| Make the files themselves smaller | **measurable** (the baseline) + **counterfactual** (the saving) | The strongest-baselined of the compression family, because the population is observed per file: which files were read, how often, and what carrying each one cost. `tokenamun what-if file-compression` ranks them by what shrinking each would have been worth, which is not the order a directory listing gives you -- a big file read on the last call barely costs anything and a small one read early costs on every call since. Unlike a proxy it shrinks the prefix itself, so it also shrinks every cache rebuild. What it cannot know is whether the shorter file would still have answered the question. |
