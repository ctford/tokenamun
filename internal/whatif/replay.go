...
func Replay(ref model.SessionRef, command string) (*ReplayResult, error) {
	...
	for {
		...
		case "user":
			...
			for _, b := range e.Message.ToolResults() {
				...
				if !eligible(names[b.ToolUseID], commands[b.ToolUseID], meta) {
					continue
				}
	...
	if result.Items == 0 {
		return result, fmt.Errorf("no eligible content was replayed successfully (%d failures)", result.Failures)
	}
	return result, nil
}
